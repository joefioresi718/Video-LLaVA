...
class LlavaMetaModel:
    ...
    def get_ssl_tower(self):
        ssl_tower = getattr(self, 'ssl_tower', None)
        if type(ssl_tower) is list:
            ssl_tower = ssl_tower[0]
        return ssl_tower
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        pretrain_ssl_mlp_adapter = model_args.pretrain_ssl_mlp_adapter
        ...
        if getattr(self, 'ssl_projector', None) is None:
            self.ssl_projector = build_vision_projector(self.config)
        else:
            # In case it is frozen by LoRA
            for p in self.ssl_projector.parameters():
                p.requires_grad = True
        ...
        if pretrain_ssl_mlp_adapter is not None:
            mm_projector_weights = torch.load(pretrain_ssl_mlp_adapter, map_location='cpu')
            def get_w(weights, keyword):
                return {k.split(keyword + '.')[1]: v for k, v in weights.items() if keyword in k}

            self.ssl_projector.load_state_dict(get_w(mm_projector_weights, 'ssl_projector'))
...
class LlavaMetaForCausalLM(ABC):
    ...
    def get_ssl_tower(self):
        return self.get_model().get_ssl_tower()
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        ssl_tower = self.get_ssl_tower()
        if (image_tower is None and video_tower is None and ssl_tower is None) or images is None or input_ids.shape[1] == 1:
            if past_key_values is not None and (image_tower is not None or video_tower is not None or ssl_tower is not None) and images is not None and input_ids.shape[1] == 1:
