...
def parse_args():
    ...
    parser.add_argument("--model_name", default="meta-llama/Meta-Llama-3-70B-Instruct", type=str, help="HuggingFace model name.")
...
def annotate(prediction_set, caption_files, output_dir, model, tokenizer, args):
    ...
    for file in caption_files:
        ...
        outputs = model.generate(inputs, max_new_tokens=256, eos_token_id=terminators, do_sample=True, temperature=0.6, top_p=0.9, pad_token_id=tokenizer.eos_token_id)
        response_message = tokenizer.decode(outputs[0][inputs.shape[-1]:], skip_special_tokens=True)
